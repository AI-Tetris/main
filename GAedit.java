...
//			return totalValue;
	...
	private int findParentIndex(float normValue, int startIndex, int endIndex) {
		...
		for (int i=0; i<popNum-1; i++) {
			...
			double nextVal = fitnessScores[i+1];
			if (normValue > val && normValue <= nextVal) {
				return i+1;
			} 
		}
...
//			double shigh = Integer.MIN_VALUE;
//			for (int z=0; z<numIterations; z++) {
//				if (shigh < p.fitnessScores[z]) {
//					shigh = p.fitnessScores[z];
//				}
//			}
//			System.out.println(Arrays.deepToString(p.population));
//			System.out.println("highest # rows cleared: " + shigh);
